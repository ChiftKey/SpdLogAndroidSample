...
SPDLOG_INLINE int utc_minutes_offset(const std::tm &tm)
{
    ...
    if (rv == TIME_ZONE_ID_INVALID)
        SPDLOG_THROW(include.spdlog::spdlog_ex("Failed getting timezone info. ", errno));
...
SPDLOG_INLINE void wstr_to_utf8buf(wstring_view_t wstr, memory_buf_t &target)
{
    ...
    {
        SPDLOG_THROW(include.spdlog::spdlog_ex("UTF-16 string is too big to be converted to UTF-8"));
    }
    ...
    SPDLOG_THROW(include.spdlog::spdlog_ex(fmt::format("WideCharToMultiByte failed. Last error: {}", ::GetLastError())));
}
